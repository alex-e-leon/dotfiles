# Generated by Powerlevel10k configuration wizard on 2020-04-20 at 17:12 AEST.
...
() {
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local       meta='%f%b'     # default foreground
      local      clean='%2F%B'   # green foreground
      local   modified='%15F%B'  # yellow foreground
      local  untracked='%15F%B'   # blue foreground
      local conflicted='%1F%B'  # red foreground
      local icon='%15F%B'  # red foreground
    else
    ...
    (( VCS_STATUS_NUM_STAGED || VCS_STATUS_NUM_UNSTAGED || VCS_STATUS_NUM_UNTRACKED || VCS_STATUS_STASHES || VCS_STATUS_HAS_UNSTAGED == -1 )) && res+=" "
    # *42 if have stashes.
    (( VCS_STATUS_STASHES        )) && res+="${icon}\$"
    # +42 if have staged changes.
    (( VCS_STATUS_NUM_STAGED     )) && res+="${icon}+"
    # !42 if have unstaged changes.
    (( VCS_STATUS_NUM_UNSTAGED   )) && res+="${icon}!"
    # ?42 if have untracked files. It's really a question mark, your font isn't broken.
    # See POWERLEVEL9K_VCS_UNTRACKED_ICON above if you want to use a different icon.
    # Remove the next line if you don't want to see untracked files at all.
    (( VCS_STATUS_NUM_UNTRACKED  )) && res+="${icon}${(g::)POWERLEVEL9K_VCS_UNTRACKED_ICON}"
    # "─" if the number of unstaged files is unknown. This can happen due to
    # POWERLEVEL9K_VCS_MAX_INDEX_SIZE_DIRTY (see below) being set to a non-negative number lower
    # than the number of files in the Git index, or due to bash.showDirtyState being set to false
    # in the repository config. The number of staged and untracked files may also be unknown
    # in this case.
    (( VCS_STATUS_HAS_UNSTAGED == -1 )) && res+="${icon}!"
  ...
  #######################[ direnv: direnv status (https://direnv.net/) ]########################
  # Direnv color.
  typeset -g POWERLEVEL9K_DIRENV_FOREGROUND=14
  # Custom icon.
